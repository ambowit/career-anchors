...
import { useBatchAssessments, useUpdateBatchStatus } from "@/hooks/useBatchAssessment";
...
export default function OrgBatchAssessmentListPage() {
  ...
  // org-level: isSuperAdmin = false, uses RLS to filter by organization
  const { data: batches = [], isLoading } = useBatchAssessments(false);
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" style={{ backgroundColor: config.bgColor, color: config.color }}>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {t("Manage your organization's batch assessments", "管理您機構的批次施測活動")}
          </p>
        </div>
        <Button onClick={() => navigate("/org/batch-assessment/create")} style={{ backgroundColor: PRIMARY }}>
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder={t("Search...", "搜尋...")} className="pl-9" />
        </div>
              ...
              {filteredBatches.map((batch) => (
                <tr key={batch.id} className="border-b last:border-b-0 hover:bg-gray-50/50 transition-colors" style={{ borderColor: "#E9ECEF" }}>
                  <td className="px-4 py-3">
                    <button onClick={() => navigate(`/org/batch-assessment/${batch.id}`)} className="font-medium hover:underline text-left" style={{ color: PRIMARY }}>
                      {batch.batch_name}
                    </button>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ backgroundColor: "#F0F2F5", color: "#555" }}>
                      {TYPE_LABELS[batch.assessment_type]?.[language] || batch.assessment_type}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => navigate(`/org/batch-assessment/${batch.id}`)}>
                          <Eye className="w-4 h-4 mr-2" /> {t("View Detail", "查看詳情")}
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        {batch.status === "draft" && (
                          <DropdownMenuItem onClick={() => handleStatusUpdate(batch.id, "active")}>
                            <PlayCircle className="w-4 h-4 mr-2" /> {t("Activate", "啟用")}
                          </DropdownMenuItem>
                        )}
                        {batch.status === "active" && (
                          <DropdownMenuItem onClick={() => handleStatusUpdate(batch.id, "paused")}>
                            <PauseCircle className="w-4 h-4 mr-2" /> {t("Pause", "暫停")}
                          </DropdownMenuItem>
                        )}
                        {batch.status === "paused" && (
                          <DropdownMenuItem onClick={() => handleStatusUpdate(batch.id, "active")}>
                            <PlayCircle className="w-4 h-4 mr-2" /> {t("Resume", "恢復")}
                          </DropdownMenuItem>
                        )}
                        {(batch.status === "active" || batch.status === "paused") && (
                          <DropdownMenuItem onClick={() => handleStatusUpdate(batch.id, "closed")} className="text-red-600">
                            <XCircle className="w-4 h-4 mr-2" /> {t("Close", "結束")}
                          </DropdownMenuItem>
                        )}
                      </DropdownMenuContent>
                    </DropdownMenu>

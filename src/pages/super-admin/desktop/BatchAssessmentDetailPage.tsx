// BatchAssessmentDetailPage - Super Admin batch assessment detail view
...
import {
  ...
  Pencil, Save, X, CheckSquare, Square, FileDown,
} from "lucide-react";
...
import {
  ...
  useBatchAccessLogs, useUpdateBatchStatus, useResetAccessCode,
  useUpdateBatch,
  type BatchResult,
...
export default function BatchAssessmentDetailPage() {
  ...
  const { data: accessLogs = [] } = useBatchAccessLogs(batchId);
  ...
  const averageScores = useMemo(() => {
    ...
    results.forEach(resultItem => {
      ...
      DIMENSION_CODES.forEach(dim => {
        totals[dim] += (scores[dim] || 0);
      });
    });
    const averages: Record<string, number> = {};
    DIMENSION_CODES.forEach(dim => {
      averages[dim] = Math.round((totals[dim] / results.length) * 10) / 10;
    });
    return averages;
  }, [results]);
  ...
  const handleResetCode = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t("Failed to reset code", "重設驗證碼失敗"));
    }
  }, [batchId]);
  ...
  const handleExportCSV = useCallback(() => {
    ...
    const rows = results.map(resultItem => [
      resultItem.participant_name,
      resultItem.department,
      resultItem.email,
      String(resultItem.work_years ?? ""),
      new Date(resultItem.completed_at).toLocaleString(),
  ...
  const handleBatchDownloadReports = useCallback(async () => {
    ...
    for (let index = 0; index < selectedResults.length; index++) {
      ...
      try {
        ...
        toast.error(t(
          `Failed: ${resultItem.participant_name}`,
          `生成失敗：${resultItem.participant_name}`,
        ));
      }
    }
    ...
    toast.success(t(
      `Downloaded ${successCount} reports as ZIP`,
      `已打包下載 ${successCount} 份報告`,
    ));
  ...
  const batchUrl = `${SITE_ORIGIN}/batch/${encodeURIComponent(batch.batch_slug)}`;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          {!isEditing && (
            <div className="flex items-center gap-3 mt-0.5">
              <p className="text-sm text-muted-foreground">{batch.organization_name}</p>
        ...
        <div className="flex items-center gap-2">
          {batch.status === "draft" && (
            <Button size="sm" onClick={() => handleStatusChange("active")} style={{ backgroundColor: "#059669" }}>
              <PlayCircle className="w-4 h-4 mr-1.5" /> {t("Activate", "啟用")}
            </Button>
          )}
          {batch.status === "active" && (
            <Button size="sm" variant="outline" onClick={() => handleStatusChange("paused")}>
              <PauseCircle className="w-4 h-4 mr-1.5" /> {t("Pause", "暫停")}
            </Button>
          )}
          {batch.status === "paused" && (
            <Button size="sm" onClick={() => handleStatusChange("active")} style={{ backgroundColor: "#059669" }}>
              <PlayCircle className="w-4 h-4 mr-1.5" /> {t("Resume", "恢復")}
            </Button>
          )}
          {(batch.status === "active" || batch.status === "paused") && (
            <Button size="sm" variant="destructive" onClick={() => handleStatusChange("closed")}>
              <XCircle className="w-4 h-4 mr-1.5" /> {t("Close", "結束")}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border p-4 space-y-3" style={{ borderColor: "#E9ECEF" }}>
          ...
          <code className="block text-xs bg-gray-50 rounded px-2 py-1.5 break-all select-all" style={{ wordBreak: "break-all" }}>{batchUrl}</code>
        ...
        <div className="bg-white rounded-xl border p-4 space-y-3" style={{ borderColor: "#E9ECEF" }}>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={handleResetCode}>
        ...
        <div className="bg-white rounded-xl border p-4 space-y-3" style={{ borderColor: "#E9ECEF" }}>
          ...
          <Button variant="outline" size="sm" className="w-full" onClick={handleCopyEmailTemplate}>
            <Mail className="w-4 h-4 mr-2" />
            {t("Copy Email Template", "複製郵件範本")}
          </Button>
        </div>
      </div>
        ...
        <TabsContent value="dashboard" className="space-y-4 mt-4">
          ...
          <div className="bg-white rounded-xl border p-5" style={{ borderColor: "#E9ECEF" }}>
            ...
            <div className="flex justify-between mt-2 text-xs text-muted-foreground">
              <span>{t("Today completed", "今日完成")}: {todayCompleted}</span>
          ...
          {recentCompletions.length > 0 && (
            <div className="bg-white rounded-xl border p-5" style={{ borderColor: "#E9ECEF" }}>
              <h3 className="text-sm font-bold mb-3" style={{ color: PRIMARY }}>
                {t("Recent Completions", "最近完成")}
              </h3>
                  ...
                  <div key={resultItem.id} className="flex items-center justify-between py-2 border-b last:border-b-0" style={{ borderColor: "#F0F2F5" }}>
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white" style={{ backgroundColor: PRIMARY }}>
                        {resultItem.participant_name.charAt(0)}
                      </div>
                      <div>
                    ...
                    <div className="text-right">
                      <p className="text-xs font-medium" style={{ color: PRIMARY }}>
                        {ANCHOR_NAMES[resultItem.main_anchor]?.[language] || resultItem.main_anchor}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {new Date(resultItem.completed_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                      </p>
                    </div>
                  </div>
                ))}
          ...
          {results.length > 0 && (
            <div className="bg-white rounded-xl border p-5" style={{ borderColor: "#E9ECEF" }}>
              <h3 className="text-sm font-bold mb-4" style={{ color: PRIMARY }}>
                {t("Average Dimension Scores", "各維度平均分")}
              </h3>
                    ...
                    <div key={dim} className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                          <div className="h-full rounded-full" style={{ width: `${score}%`, backgroundColor: score >= 65 ? "#059669" : score >= 45 ? "#2980B9" : "#95A5A6" }} />
                        </div>
                      </div>
        ...
        <TabsContent value="results" className="space-y-4 mt-4">
              ...
              >
                {isDownloadingReports ? (
                  <>
                    ...
                    {t(`${downloadProgress.current}/${downloadProgress.total}`, `${downloadProgress.current}/${downloadProgress.total}`)}
                  </>
                ) : (
                  <>
          ...
          {results.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-muted-foreground">{t("No results yet", "尚無結果")}</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b" style={{ borderColor: "#E9ECEF", backgroundColor: "#FAFBFC" }}>
                    ...
                    <th className="text-left px-4 py-3 font-medium text-muted-foreground whitespace-nowrap">{t("Name", "姓名")}</th>
                    <th className="text-left px-4 py-3 font-medium text-muted-foreground whitespace-nowrap">{t("Department", "部門")}</th>
                    <th className="text-left px-4 py-3 font-medium text-muted-foreground whitespace-nowrap">{t("Work Yrs", "年資")}</th>
                    <th className="text-left px-4 py-3 font-medium text-muted-foreground whitespace-nowrap">{t("Completed", "完成時間")}</th>
                    <th className="text-left px-4 py-3 font-medium text-muted-foreground whitespace-nowrap">{t("Main Anchor", "主錨")}</th>
                    {DIMENSION_CODES.map((dim) => (
                      <th key={dim} className="text-center px-2 py-3 font-medium text-muted-foreground whitespace-nowrap">{dim}</th>
                    ))}
                    <th className="text-right px-4 py-3 font-medium text-muted-foreground whitespace-nowrap">{t("Action", "操作")}</th>
                  </tr>
                </thead>
                <tbody>
                  {results.map((resultItem) => (
                    <tr key={resultItem.id} className={`border-b last:border-b-0 transition-colors ${selectedResultId === resultItem.id ? "bg-blue-50/70" : "hover:bg-gray-50/50"}`} style={{ borderColor: "#E9ECEF" }}>
                      <td className="w-10 px-3 py-3">
                        <Checkbox
                          checked={selectedIds.has(resultItem.id)}
                          onCheckedChange={() => toggleSelect(resultItem.id)}
                        />
                      </td>
                      <td className="px-4 py-3 font-medium whitespace-nowrap">{resultItem.participant_name}</td>
                      <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{resultItem.department || "—"}</td>
                      <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{resultItem.work_years ?? "—"}</td>
                      <td className="px-4 py-3 text-xs text-muted-foreground whitespace-nowrap">
                        {new Date(resultItem.completed_at).toLocaleString([], { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: PRIMARY + "10", color: PRIMARY }}>
                          {resultItem.main_anchor}
                        </span>
                      </td>
                      {DIMENSION_CODES.map((dim) => {
                        const score = resultItem.calculated_scores?.[dim] ?? 0;
                        return (
                          <td key={dim} className="px-2 py-3 text-center text-xs tabular-nums" style={{ color: score >= 65 ? "#059669" : "#555" }}>
                            {score}
                          </td>
                        );
                      })}
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <Button variant="ghost" size="sm" className="h-7 px-2" onClick={() => setSelectedResultId(selectedResultId === resultItem.id ? null : resultItem.id)}>
                            {selectedResultId === resultItem.id ? <EyeOff className="w-3.5 h-3.5 text-primary" /> : <Eye className="w-3.5 h-3.5" />}
                          </Button>
                          <Button variant="ghost" size="sm" className="h-7 px-2" disabled={downloadingItemId === resultItem.id} onClick={() => handleSingleDownload(resultItem)}>
                            {downloadingItemId === resultItem.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Download className="w-3.5 h-3.5" />}
                          </Button>
                        </div>
                      </td>
                    </tr>
          ...
          {selectedResultId && (() => {
            const selectedResult = results.find(resultItem => resultItem.id === selectedResultId);
            if (!selectedResult) return null;
            const scores = selectedResult.calculated_scores || {};
            return (
              <div ref={detailPanelRef} className="bg-white rounded-xl border p-6 animate-in fade-in slide-in-from-top-2 duration-200" style={{ borderColor: PRIMARY + "30", boxShadow: `0 0 0 1px ${PRIMARY}15, 0 4px 12px ${PRIMARY}08` }}>
                <h3 className="text-sm font-bold mb-4" style={{ color: PRIMARY }}>
                  {selectedResult.participant_name} — {t("Detail", "詳情")}
                </h3>
                    ...
                    return (
                      <div key={dim} className="rounded-lg border p-3" style={{ borderColor: "#E9ECEF" }}>
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs font-bold" style={{ color: PRIMARY }}>{dim}</span>
                        ...
                        <div className="h-1.5 bg-gray-100 rounded-full mt-2 overflow-hidden">
                          <div className="h-full rounded-full" style={{ width: `${score}%`, backgroundColor: levelColor }} />
                        </div>
                      </div>
                    );
                  })}
        ...
        <TabsContent value="team" className="space-y-4 mt-4">
          {results.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-muted-foreground">{t("Not enough data for team stats", "數據不足，無法進行團隊統計")}</p>
            </div>
          ) : (
            <>
              <div className="bg-white rounded-xl border p-5" style={{ borderColor: "#E9ECEF" }}>
                <h3 className="text-sm font-bold mb-4" style={{ color: PRIMARY }}>
                  {t("Main Anchor Distribution", "主錨點分布")}
                </h3>
                <div className="space-y-2">
                  {anchorDistribution.map(([anchor, count]) => {
                    ...
                    return (
                      <div key={anchor} className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          <div className="h-6 bg-gray-50 rounded overflow-hidden relative">
                            <div className="h-full rounded" style={{ width: `${percent}%`, backgroundColor: PRIMARY + "20" }} />
                            <span className="absolute left-2 top-1/2 -translate-y-1/2 text-xs font-medium" style={{ color: "#333" }}>
                              {ANCHOR_NAMES[anchor]?.[language] || anchor}
                            </span>
                          </div>
                        </div>
                        <div className="w-16 text-right text-xs">
                          <span className="font-bold" style={{ color: PRIMARY }}>{count}</span>
              ...
              <div className="bg-white rounded-xl border p-5" style={{ borderColor: "#E9ECEF" }}>
                <h3 className="text-sm font-bold mb-4" style={{ color: PRIMARY }}>
                  {t("Department Distribution", "部門分布")}
                </h3>
              ...
              <div className="bg-white rounded-xl border p-5" style={{ borderColor: "#E9ECEF" }}>
                <h3 className="text-sm font-bold mb-4" style={{ color: PRIMARY }}>
                  {t("Work Experience Distribution", "年資分布")}
                </h3>
                <div className="grid grid-cols-4 gap-3">
                  {Object.entries(workYearsDistribution).map(([range, count]) => {
                    ...
                    return (
                      <div key={range} className="bg-gray-50 rounded-lg p-3 text-center">
                        ...
                        <p className="text-xl font-bold mt-1" style={{ color: PRIMARY }}>{count}</p>
              ...
              <div className="bg-white rounded-xl border p-5" style={{ borderColor: "#E9ECEF" }}>
                <h3 className="text-sm font-bold mb-4" style={{ color: PRIMARY }}>
                  {t("Team Average Scores", "團隊平均分數")}
                </h3>
                <div className="space-y-2.5">
                  {DIMENSION_CODES.map((dim) => {
                    const score = averageScores[dim] || 0;
                    const levelColor = score >= 65 ? "#059669" : score >= 45 ? "#2980B9" : "#95A5A6";
                    return (
                      <div key={dim} className="flex items-center gap-3">
                        <div className="w-20 text-xs">
                          <span className="font-bold" style={{ color: PRIMARY }}>{dim}</span>
                          <span className="text-muted-foreground ml-1 hidden sm:inline">
                            {ANCHOR_NAMES[dim]?.[language]?.substring(0, 6) || ""}
                          </span>
                        </div>
                        <div className="flex-1">
                          <div className="h-4 bg-gray-100 rounded-full overflow-hidden">
                            <div className="h-full rounded-full transition-all" style={{ width: `${score}%`, backgroundColor: levelColor }} />
                          </div>
                        </div>
        ...
        <TabsContent value="logs" className="space-y-4 mt-4">
          {accessLogs.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-muted-foreground">{t("No logs yet", "尚無日誌")}</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b" style={{ borderColor: "#E9ECEF", backgroundColor: "#FAFBFC" }}>
                    ...
                    <th className="text-left px-4 py-3 font-medium text-muted-foreground">{t("Event", "事件")}</th>
                    <th className="text-left px-4 py-3 font-medium text-muted-foreground">{t("Details", "詳情")}</th>
                  </tr>
                </thead>
                <tbody>
                  {accessLogs.map((log: any) => {
                    const eventLabels: Record<string, string> = {
                      link_visit: t("Link visited", "連結訪問"),
                      code_verify_success: t("Code verified", "驗證碼成功"),
                      code_verify_fail: t("Code failed", "驗證碼失敗"),
                      assessment_start: t("Assessment started", "開始測評"),
                      assessment_complete: t("Assessment completed", "完成測評"),
                      batch_created: t("Batch created", "批次建立"),
                      batch_paused: t("Batch paused", "批次暫停"),
                      batch_closed: t("Batch closed", "批次結束"),
                    };
                    return (
                      <tr key={log.id} className="border-b last:border-b-0" style={{ borderColor: "#E9ECEF" }}>
                        <td className="px-4 py-3 text-xs text-muted-foreground whitespace-nowrap">
                        ...
                        <td className="px-4 py-3">
                          <span className="text-xs font-medium px-2 py-0.5 rounded-full" style={{
                            backgroundColor: log.event_type?.includes("fail") ? "#FEF2F2" : log.event_type?.includes("complete") ? "#ECFDF5" : "#F3F4F6",
                            color: log.event_type?.includes("fail") ? "#DC2626" : log.event_type?.includes("complete") ? "#059669" : "#6B7280",
                          }}>
                        ...
                        <td className="px-4 py-3 text-xs text-muted-foreground">
                          {log.metadata ? (() => {
                            ...
                            const parts: string[] = [];
                            if (meta.created_by) parts.push(`${t("By", "操作者")}：${meta.created_by}`);
                            if (meta.participant_name) parts.push(`${t("Name", "姓名")}：${meta.participant_name}`);
                            if (meta.attempt) parts.push(`${t(`Attempt #${meta.attempt}`, `第 ${meta.attempt} 次嘗試`)}`);
                            if (meta.session_id) parts.push(`${t("Session", "場次")}：${String(meta.session_id).slice(0, 8)}`);
                            if (meta.duration_seconds) {
                              const mins = Math.floor(Number(meta.duration_seconds) / 60);
                              const secs = Number(meta.duration_seconds) % 60;
                              parts.push(`${t("Duration", "耗時")}：${mins > 0 ? `${mins}m ` : ""}${secs}s`);
                            }
                            // Fallback for unknown keys
                            if (parts.length === 0) {
                              Object.entries(meta).forEach(([key, value]) => parts.push(`${key}：${value}`));
                            }
                            return parts.join("　");
                          })() : "—"}
                        </td>

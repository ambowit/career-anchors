...
serve(async (req) => {
  ...
  try {
    ...
    // Use the REST API to run SQL via rpc if available, or use pg directly
    // We'll use the Supabase management API approach - execute raw SQL
    ...
    const sql = `
      CREATE TABLE IF NOT EXISTS anonymous_assessment_batches (
        ...
        report_visibility TEXT NOT NULL DEFAULT 'personal_only',
    ...
    const pool = new pgModule.Pool(dbUrl, 1, true);
    ...
    try {
      ...
      console.log("[setup-anon-tables] All tables created + schema reloaded");
    } finally {
      connection.release();
      await pool.end();
    }

    return new Response(JSON.stringify({ success: true, message: "All anonymous assessment tables created" }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 200,
    });
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error);
    console.error("[setup-anon-tables] ERROR:", msg);

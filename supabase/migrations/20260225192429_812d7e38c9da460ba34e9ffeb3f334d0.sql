-- Create a SECURITY DEFINER function to handle demo organization + profile setup
-- This bypasses RLS so test accounts can bootstrap themselves
...
BEGIN
  ...
  INSERT INTO profiles (id, email, full_name, role, role_type, avatar_url, status, organization_id, updated_at)
  VALUES (p_user_id, p_email, p_full_name, p_role, p_role_type, '', 'active', v_org_id, now())
...
COMMENT ON FUNCTION public.setup_test_account IS 'Bootstrap a test account with proper role and optional demo organization (bypasses RLS)';
